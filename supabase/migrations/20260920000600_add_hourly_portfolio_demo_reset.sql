-- Portfolio-demo baseline reset. The UUIDs below come directly from
-- supabase/demo-baseline.json and intentionally exclude canonical goals.
-- Baseline: 2 tutors, 5 students, 5 assignments, 7 sessions, and 9 session goals.
...
-- This migration is run from Supabase SQL Editor as postgres.  The function
-- therefore runs with postgres privileges, while the browser roles cannot invoke it.
alter function public.reset_portfolio_demo_data() owner to postgres;

revoke execute on function public.reset_portfolio_demo_data() from public;
revoke execute on function public.reset_portfolio_demo_data() from anon, authenticated;

select cron.schedule(
  'reset-portfolio-demo-hourly',
  '0 * * * *',
  'select public.reset_portfolio_demo_data();'
);
